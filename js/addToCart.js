...
function addCartClicked(res) {
  ...
  function createEl(allCartData, thisId) {
    allCartData.forEach((cartItem) => {
      // if (cartItem.id == thisId) {
      const cartElClone = document.importNode(cartTemplate.content, true)
      cartElClone
        .querySelector('.cart-quantity')
        .setAttribute('value', cartItem.quantity)
      cartElClone.querySelector('.cart-price').textContent = cartItem.price
      cartElClone.querySelector('.cart-product-title').textContent = cartItem.name
